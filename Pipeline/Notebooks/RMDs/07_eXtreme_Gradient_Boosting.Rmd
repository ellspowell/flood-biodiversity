---
title: "07 Extreme Gradient Boosting"
author: "EP"
date: "2026-07-19"
output:
...
As seen in the Random Forest models, `n_days` is dominant predictor. For species richness, the full model has an ROC AUC of 0.956 dropping to 0.706 in the ND model (no `n_days`). The full Simpson model has an ROC AUC of 0.712, dropping to 0.662 in the ND model.
...
## Classification and CV Fold Setup
```{r}
#Classification
...
richness_recipe_xgb <- recipe(
  ...
  # remove observations missing numerical predictors 
  step_naomit(all_numeric_predictors(), all_outcomes()) %>%
  # allow previously unseen categorical levels in new data
...
xgb_grid <- grid_space_filling(
  trees(range = c(100, 1000)), # Number of boosting trees
  tree_depth(range = c(2, 8)), # Max depth of each tree
  learn_rate(range = c(-3, -0.5)),   # Learning rate, log10 scale 
  loss_reduction(range = c(-5, 1.5)), # Min improvement for further tree split (also log10 scale)
  finalize(mtry(range = c(2, 8)), df[, predictors]), # Number of predictors at each split, finalised based on number of predictors
  min_n(range = c(2, 15)), # Min obs at each node
  sample_prop(range = c(0.5, 1.0)), # Proportion of training observations
  size = 30,
  type = "latin_hypercube") # 30 combos using latin hypercube sampling

#Latin hypercube sampling used for efficient, equal parameter testing without testing every possibility.
```
...
## Prediction Rate AUC
```{r}
# Get overall mean ROC AUC for richness and Simpson
overall_richness_xgb <- collect_metrics(richness_res_xgb) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(
    best_richness_xgb,
    by = names(best_richness_xgb)) %>%
  pull(mean)
...
overall_simpson_xgb <- collect_metrics(simpson_res_xgb) %>%
  ...
  semi_join(
    best_simpson_xgb,
    by = names(best_simpson_xgb)) %>%
  pull(mean)

# Get city based ROC AUC scores (by fold, then attach city name)
city_richness_xgb <- collect_metrics(
  richness_res_xgb,
  summarize = FALSE) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(
  ...
  select(city, richness_auc = .estimate)

city_simpson_xgb <- collect_metrics(
  simpson_res_xgb,
  summarize = FALSE) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(
    ...
    by = names(best_simpson_xgb)) %>%
  left_join(fold_city_lookup, by = "id") %>%
  select(city, simpson_auc = .estimate)

# Combine richness and Simpson city scores
city_auc_xgb <- full_join(city_richness_xgb, city_simpson_xgb,
                          by = "city")

# Create row for overall score
...
classification_auc_table_xgb <- bind_rows(
  overall_auc_xgb,city_auc_xgb) %>%
...
classification_auc_table_xgb %>%
  knitr::kable(
    caption = paste(
      "Overall and city-level cross-validated ROC AUC for",
...
## Set up and tuning 
```{r}
# Set new predictors without `n_days`
predictors_ND <- c("dtm_elev", "avg_imperv", "rofsw_risk", "rofrs_risk",
                "avg_area_ha", "treeht", "weighted_precip", "weighted_temp",
                "dist_to_river", "pct_greenspace")

#Recipes and workflows, as before.
...
overall_richness_xgb_ND <- collect_metrics(richness_res_xgb_ND) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(best_richness_xgb_ND, 
            by = names(best_richness_xgb_ND)) %>%
  pull(mean)

overall_simpson_xgb_ND <- collect_metrics(simpson_res_xgb_ND) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(best_simpson_xgb_ND, 
            by = names(best_simpson_xgb_ND)) %>%
  pull(mean)

city_richness_xgb_ND <- collect_metrics(
  richness_res_xgb_ND,
  summarize = FALSE) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(best_richness_xgb_ND, 
...
city_simpson_xgb_ND <- collect_metrics(
  simpson_res_xgb_ND,
  summarize = FALSE) %>%
  filter(.metric == "roc_auc") %>%
  semi_join(best_simpson_xgb_ND, 
...
richness_recipe_xgb_ND_prepped <- extract_recipe(richness_fit_xgb_ND)
saveRDS(richness_recipe_xgb_ND_prepped, "Data/richness_recipe_xgb_ND.rds")
#saveRDS(richness_fit_xgb_ND, "richness_fit_xgb_ND.rds")
```
...
## ROC Curves
```{r}
p1 <- roc_curve(richness_success_xgb_ND, truth = richness_class, .pred_high, 
          event_level = "second") %>%
  autoplot() + labs(title = "Success rate curve: ND species richness (XGBoost)")

p2 <- roc_curve(simpson_success_xgb_ND, truth = simpson_class, .pred_high, 
          event_level = "second") %>%
...
#ggsave("xgb_ND_rich_ROC.png", p1, width = 12, height = 5, dpi = 300)
#ggsave("xgb_ND_simp_ROC.png", p2, width = 12, height = 5, dpi = 300)
```
...
#ggsave("VI_xgb_ND_rich.png", p3, width = 12, height = 5, dpi = 300)
#ggsave("VI_xgb_ND_simp.png", p4, width = 12, height = 5, dpi = 300)
```
