...
```
The richness model has a best ROC AUC of 0.951, and performance is stable across the grid. This suggests that the model is not highly sensitive to tuning. However, the simpson model is noticably weaker (ROC AUC = 0.710) and more variablility between tuning models. This suggests that the richness model classifies better than the simpson diversity using these predictor variables.
...
simpson_na_folds
```
As seen before, there is more variability in the Simpson scores compared to the richness scores when looking at individual cities. This suggests that the richness model extrapolates to other cities better than the simpson model. 
...
```
For the richness model, there is a small gap between the sample AUC and cross-city AUC (0.984 to 0.951) suggesting that the model is capturing real patterns, not sure memorizing city specific patterns. On the other hand, the Simpson model has a larger gap (0.933 to 0.710). This suggests that the model fits the training data well, but struggles to generalise to other, unseen, cities. This is backed up by the previous, city level breakdown of AUC scores where there was more variablity in the Simpson scores. 
...
simpson_fit_class %>%
  ...
  labs(title = "Variable importance: Simpson diversity classification",
       x = NULL, y = "Permutation importance")
```
Once again, `n_days` is the dominant predictor variable for both models, likely representing sampling effort over true biodiversity. In the richness model, `weighted_temp`, `pct_greenspace`, and `avg_area_ha` show secondary importance. `weighted_temp` and `pct_greenspace` were also relevant in the regression random forest and significant in the negative binomial regression. In the Simpson model, `pct_greenspace`, `avg_imperv`, `dist_to_river`, and `avg_area_ha` show secondary importance. 

To conclude, the richness classifier model is strong and demonstrates transferability between cities. There is evidence that the predictors capture real ecological signal. Whereas the Simpson model is weaker with the training data fitting well but it struggles to generalise to new cities. 
To attempt to look closer at the true ecological drivers, as opposed to sampling effort, these models are rerun in the next notebook without the predictor `n_days`. 
